...
async fn main() {
    ...
    if std::env::var("SETUP_PERMISSIONS").unwrap_or_default() == "true" {
        ...
        StrategyEngine::grant_sell_permission().await;
        // Important: Remove the lock before exiting so you don't have to manually 'rm' it
        let _ = fs::remove_file(lock_file); 
        println!("[SETUP MODE] Complete. Exiting.");
        std::process::exit(0); 
    }

    // MARKET_ASSET: "btc" (default) or "eth"
    ...
    let binance_ws_url = env::var("BINANCE_WS_URL").unwrap_or_else(|_| {
        match market_asset.as_str() {
            "eth" => "wss://stream.binance.com:9443/ws/ethusdt@ticker".to_string(),
            _     => "wss://stream.binance.com:9443/ws/btcusdt@ticker".to_string(),
        }
    });
                    ...
                    while let Some(msg) = read.next().await {
                        if let Ok(Message::Text(text)) = msg {
                            if let Ok(json) = serde_json::from_str::<serde_json::Value>(&text) {
                                if let Some(p_str) = json["c"].as_str() {
